...
class DatabaseManager:
    ...
    def save_credential(
        ...
        - Prevents duplicates at the DB level:
          same user_id + same title will be rejected.
        """
        try:
            # Duplicate check
            ...
            if existing:
                # Duplicate credential for this user
                return False

           
            encrypted_data = self.crypto.encrypt_credential_data(
                credential_data,
                master_password
            )
    ...
    def get_user_credentials(
        ...
        for cred in credentials:
            try:
                decrypted_data = self.crypto.decrypt_credential_data(
                    cred.encrypted_data,
                    master_password
                )
